KMPVVMCM ;ATL/JMF - Collect HL7/HLO counts for the VistA Message Count Monitor ;Jan 13, 2020@17:16
...
RUN ; Collect metrics per configured interval and store in ^KMPTMP("KMPV","VMCM","DLY" -- CALLED VIA CACHE TASK MANAGER
 ...
 ..N H S H="Time Slot^Node^Logical Link^Last Incoming Message^"
 ..S H=H_"First Incoming Message^Last Outgoing Message^First Outgoing Message^"
 ..S H=H_"Link State"
 ..D HEAD^KMPVLOG(H,"KMPV","VMCM",1)  ; / *10003*
 ...
 .;
 .I '$$VA^KMPVLOG D  ; *10003* Write to file outside VA
 ..N KMPOUT,C
 ..S C=0
 ..N LNAME S LNAME=""
 ..F  S LNAME=$O(^KMPTMP("KMPV","VMCM","DLY",+KMPVH,KMPVNODE,LNAME)) Q:LNAME=""  D
 ...S C=C+1
 ...S KMPOUT(C)=KMPVHTIME_U_KMPVNODE_U_LNAME_U
 ...S KMPOUT(C)=KMPOUT(C)_^KMPTMP("KMPV","VMCM","DLY",+KMPVH,KMPVNODE,LNAME,KMPVHTIME)
 ..D EN^KMPVLOG("KMPOUT","KMPV","VMCM","A",1)
 ..K ^KMPTMP("KMPV","VMCM") ; /*10003* END
 .;
 .H KMPVHANG
 Q
 ;
...
SEND ; Format and send data to CPE once a day -- TASKED VIA TASKMAN
 I '$$VA^KMPVLOG QUIT  ; *10003* Outside of the VA don't send anything
